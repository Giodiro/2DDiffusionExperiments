...
class ScoreIdentityDistillation(PretrainedFlowDistiller):
    def __init__(
        self,
        ...
        self.alpha = alpha
        self.only_loss_one = only_loss_one
    ...
    def loss(
        ...
        t = sample_time(gen_samples.shape, gen_samples.device, generator=self.rng)
        ...
        loss_one = ((v_teacher - v_auxiliary) ** 2)
        loss_two = ((v_teacher - v_auxiliary) * (v_teacher - (gen_samples - noise_samples)))
        if self.only_loss_one:
            loss_batch = loss_one
        else:
            loss_batch = loss_two - self.alpha * loss_one
        loss_weight = torch.abs((gen_samples - noise_samples) - v_teacher).sum(dim=data_dims, keepdim=True).clip(min=1e-5).detach()
        loss = torch.mean(torch.sum(loss_batch / loss_weight, dim=data_dims))
        return loss
